...
SUBROUTINE prepoststep_ens_pdaf(step, dim_p, dim_ens, dim_ens_p, dim_obs_p, &
  ...
  USE mod_model, &           ! Model variables
       ONLY: nx, rmse_, spread_, total_steps
...
! ************************************************************
  ...
  if (mod(step, delt_obs) == 0) then
    rmse_(step/delt_obs) = rmse_true
    spread_(step/delt_obs) = rmserror_est
  end if
...
! *******************
  ...
  ! write rmse to file
  filename = 'results/rmserror_'//TRIM(filter_type)//'_'// &
 ...
 ! write spread to file
  filename = 'results/spread_'//TRIM(filter_type)//'_'// &
             TRIM(exp_type)//'_'//TRIM(exp_var_spec)//'.txt'
