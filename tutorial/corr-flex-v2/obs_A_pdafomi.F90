...
! *********************************************
    ...
    IF (mype_filter==0) &
         WRITE (*,'(8x,a)') 'Assimilate observations'
...
! **********************************
    ...
    obs_field = reshape(obs_arr, (/nx,nx/))
...
! ***********************************************************
    ...
    cnt = 0
    DO i = 1, nx*nx
        IF (obs_arr(i) > -999.0) THEN
            cnt = cnt + 1
            thisobs%id_obs_p(1, cnt) = i
            obs_p(cnt) = obs_arr(i)
            ocoord_p(1, cnt) = REAL(MOD(i-1, nx) + 1)
            ocoord_p(2, cnt) = REAL((i - 1) / nx + 1)
        END IF
    END DO
